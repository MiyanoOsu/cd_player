...
FLACDecoderResult FLACDecoder::decode_frame(int32_t *output_buffer,
  ...
  *num_samples = block_size * this->num_channels_;
  ...
  // Copy samples to output buffer (now int32_t)
...
void FLACDecoder::restore_linear_prediction(const std::vector<int32_t> &coefs,
  ...
  for (std::size_t i = coefs.size(); i < this->block_result_.size(); i++) {
    // FIX: Use int64_t to prevent overflow during 24-bit decoding
    int64_t sum = 0;
    for (std::size_t j = 0; j < coefs.size(); j++) {
      // Cast to int64_t before multiplying
      sum += ((int64_t)this->block_result_[i - 1 - j] * coefs[j]);
    }
